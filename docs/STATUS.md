...
OrchAI is currently in the `v0.4.1` implementation stage as of
`2026-09-10`.
...
| AI Provider Streaming (execute_stream) | Task-bounded streaming execution | implemented | `ExecutionEngine.run_stream()` reassembles chunks into one atomic terminal result; reachable via `POST /executions/{id}/run-stream` and, through the orchestrator's streaming counterpart, `POST /requests/{id}/approve-stream` (both SSE, API-only, CLI intentionally non-streaming); the Desktop Approval Card consumes the latter |
...
| Windows Local Setup Launcher | Guided first-run environment/dependency check for a Windows checkout | in_progress | `tools/windows/orchai-setup.bat` (interactive menu plus `check [headless\|desktop]`), adapted from OrchFlow's `orchflow-setup.bat`; a unified root `orchai.bat`, `orchai-control.bat` process control, and a bootstrap executable remain planned |
...
- The full test suite is 301 tests, `uv run ruff check` clean, `uv lock --check` consistent.
